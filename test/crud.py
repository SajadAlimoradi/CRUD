...
class CrudTestCase(unittest.TestCase):
    ...
    def test_create_user(self, mock_input):
        ...
        created_student = session.query(Student).filter_by(student_number='1234567891').first()
        ...
        self.assertEqual(created_student.date_of_birth.isoformat(), '1990-01-01')
        self.assertEqual(created_student.educational_level, 'Bachelor')
        self.assertEqual(created_student.registration_date.isoformat(), '2021-01-01')
        self.assertEqual(created_student.graduation_date.isoformat(), '2025-01-01')
    ...
    def test_update_user(self, mock_input):
        mock_input.side_effect = [
            '1234567891',  # Enter student number to update
            ...
            'Updated Address',  # Enter updated address
        ...
        updated_student = session.query(Student).filter_by(student_number='1234567891').first()

        # Add assertions to check if the student was updated correctly
        self.assertIsNotNone(updated_student)
        self.assertEqual(updated_student.first_name, 'John')  # First name should remain unchanged
        self.assertEqual(updated_student.last_name, 'Updated Last Name')
        self.assertEqual(updated_student.gender, 'Female')
        self.assertEqual(updated_student.date_of_birth.isoformat(), '1990-01-01')  # Date of birth should remain unchanged
        self.assertEqual(updated_student.educational_level, 'Master')
        self.assertEqual(updated_student.registration_date.isoformat(), '2021-01-01')  # Registration date should remain unchanged
        self.assertEqual(updated_student.graduation_date.isoformat(), '2026-01-01')
        self.assertEqual(updated_student.address, 'Updated Address')
        self.assertEqual(updated_student.phone_number, '09181234578')

    @patch('builtins.input')
    def test_delete_user_found(self, mock_input):
        mock_input.return_value = '1234567891'

        crud = Crud('delete')
        crud.delete_user()

        # Add assertions to check if the user was deleted from the database
        deleted_student = session.query(Student).filter_by(student_number='1234567890').first()
    ...
    def test_delete_user_not_found(self, mock_input):
        ...
        # For example, assert that the error message "No student found with the provided student number." is logged
